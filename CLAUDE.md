...
### Web typing pad (`crates/wasm-adapter/web/`)

Original, bundler-free HTML/CSS/JS (`index.html`/`style.css`/`app.js`) on top of the `wasm-adapter` npm package: a typing canvas rendering committed text plus a live underlined preedit span, a Bangla/English toggle (bypasses the engine entirely and inserts raw characters when off), a suggestion strip (`suggestions()`, click or digit keys 1-5 to `commit_suggestion`), and a `localStorage`-backed drafts panel (save/load/delete). `AvroState` (`wasm-adapter/src/lib.rs`) has `load_dict`/`load_suffix_dict` so the page constructs the engine immediately with just the grammar (typing works right away) and loads the ~7 MB dictionary in the background afterward without blocking input. `make web` (root `Makefile`) runs `make wasm` then stages `avro.json`/`avrodict.js`/`suffixdict.js` into `crates/wasm-adapter/web/data/` (gitignored, like `pkg/`) so the directory is a self-contained static root — serve with `python3 -m http.server --directory crates/wasm-adapter/web`.

This directory is licensed MPL-1.1, not MIT like the rest of the workspace — see `crates/wasm-adapter/web/LICENSE.md` — crediting OmicronLab's [AvroPad](https://github.com/omicronlab/avro-pad) as design inspiration (an original implementation, not a copy — no AvroPad source/assets were used). Don't assume workspace-wide MIT applies inside that directory.
