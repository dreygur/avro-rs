fn main() {
    ...
    let pkgdatadir = std::env::var("PKGDATADIR")
        .unwrap_or_else(|_| "/usr/share/fcitx5/avro".to_string());
    build.define("PKGDATADIR", Some(format!("\"{pkgdatadir}\"").as_str()));

    for path in fcitx_core.include_paths.iter().chain(fcitx_utils.include_paths.iter()) {
        build.include(path);
    }
    ...
    for path in fcitx_core.link_paths.iter().chain(fcitx_utils.link_paths.iter()) {
